...
import { RosterTableData } from "./RosterColumns"

interface RosterRowActionsProps {
  row: Row<RosterTableData>
}

export function RosterRowActions({ row }: RosterRowActionsProps) {
  const { updateEntry, deleteEntry } = useRosterStore()
  ...
  const onEdit = async () => {
    try {
      ...
      // updateEntry(String(row.original.id), row.original as Partial<Tables<"associates">>)
